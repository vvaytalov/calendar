import { Box, Button, Card, CardActions, CardContent, Checkbox, Chip, FormControlLabel, Paper, Stack, Typography } from '@mui/material';
import { DeleteOutline, EditOutlined } from '@mui/icons-material';
import { cardSx, panelSx, sectionTitleSx } from './styles';

export function BaseCards({ items, onEdit, onDelete }) {
          ...
          <Card key={item.id} variant="outlined" sx={{ ...cardSx, position: 'relative' }}>
            ...
            <CardContent sx={{ px: 1.25, py: 0.9, '&:last-child': { pb: 0.9 } }}>
              ...
              <Stack direction="row" spacing={0.25} mt={0.5} flexWrap="wrap">
                {item.days.map((day) => (
                  <Chip key={`${item.id}-${day}`} size="small" label={day} sx={{ height: 18, fontSize: 10, backgroundColor: '#F0FDF4' }} />
                ))}
              </Stack>
            </CardContent>
            <CardActions sx={{ px: 1.25, py: 0.5 }}>
              <Button size="small" startIcon={<EditOutlined fontSize="small" />} onClick={() => onEdit(item.id)} sx={{ color: '#16A34A' }}>Изменить</Button>
